...
class TrainDataValidator:
    def __init__(self, pkl_file, train_dir, label_file):
    ...
    def validate_file_existence(self):
        ...
        if not os.path.exists(self.train_dir):
            self.log_error(f"{self.train_dir} does not exist")
            return False
        self.log_pass(f"{self.train_dir} exists")
    ...
    def validate_structure(self):
        ...
        if 'split' in self.data:
            ...
            elif 'train' not in self.data['split']:
                self.log_error("'split' missing 'train' key")
            else:
                self.log_pass(f"Split has 'train' key with {len(self.data['split']['train'])} samples")
    ...
    def validate_labels(self):
        ...
        if mismatches:
            ...
            self.log_pass("All labels match train_labels.txt")
        
        if missing:
            self.log_error(f"Found {len(missing)} frame_dirs without labels in train_labels.txt")
            for m in missing[:5]:
    ...
    def validate_split_consistency(self):
        ...
        split_dirs = set(self.data['split']['train'])
        ...
        if len(split_dirs) != len(self.data['split']['train']):
    ...
    def validate_filesystem_consistency(self):
        ...
        for annotation in self.data['annotations']:
            ...
            folder_path = os.path.join(self.train_dir, frame_dir)
    ...
    def validate_data_integrity(self):
        ...
        for idx, annotation in enumerate(self.data['annotations']):
            frame_dir = annotation['frame_dir']
            pose_file = os.path.join(self.train_dir, frame_dir, 'pose_3d.npy')
    ...
    def run_all_validations(self):
        print("\n" + "="*60)
        print("TRAIN.PKL COMPREHENSIVE VALIDATION")
        print("="*60)
